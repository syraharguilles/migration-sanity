...
import type {WP_REST_API_Brands} from '../types/WP_REST_API_Brands'; // Adjust the path
import type {Brands} from '../../../sanity.types'
...
type StagedBrands = Omit<Brands, '_createdAt' | '_updatedAt' | '_rev'>


export async function transformToBrands(
  wpDoc: WP_REST_API_Brands,
  client: SanityClient,
  existingImages: Record<string, string> = {},
): Promise<StagedBrands> {
  const doc: StagedBrands = {
    _id: `brands-${wpDoc.id}`,
    _type: 'brands',
  }
  ...
  if (wpDoc.status) {
    doc.status = wpDoc.status as StagedBrands['status']
  }
